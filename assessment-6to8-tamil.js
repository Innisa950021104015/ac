...
const isPsychometric = document.location.pathname.includes('psychometric') || document.location.pathname.includes('tamil-psychometric-6to8.html');
const isActivity = document.location.pathname.includes('activity') || document.location.pathname.includes('tamil-activity-6to8.html');
...
const careerDomains = {
    arts: {
        ...
        careers: [
            { title: 'Dancer', code: '27-2031.00', description: 'Perform dances to entertain people.' },
            { title: 'Photographer', code: '27-4021.00', description: 'Take pictures to capture special moments.' },
            { title: 'Writer', code: '27-3043.00', description: 'Write stories or books for others to read.' },
            { title: 'Actor', code: '27-2011.00', description: 'Act in plays or movies to tell stories.' },
            { title: 'Craft Artist', code: '27-1012.00', description: 'Make beautiful things like pottery or jewelry.' }
        ]
    },
    science: {
        riasec: ['I', 'R'],
        ...
        careers: [
            { title: 'Veterinarian', code: '29-1131.00', description: 'Take care of animals and help them stay healthy.' },
            { title: 'Biologist', code: '19-1020.01', description: 'Study plants and animals to learn about nature.' },
            { title: 'Dentist', code: '29-1021.00', description: 'Help people keep their teeth clean and healthy.' },
            { title: 'Environmental Scientist', code: '19-2041.00', description: 'Protect nature by studying the environment.' },
            { title: 'Chemist', code: '19-2031.00', description: 'Mix chemicals to discover new things.' }
        ]
    },
    computer: {
        riasec: ['I', 'C'],
        ...
        careers: [
            { title: 'Web Developer', code: '15-1254.00', description: 'Build websites for people to use online.' },
            { title: 'Game Designer', code: '15-1255.00', description: 'Create fun video games for kids to play.' },
            { title: 'IT Support Specialist', code: '15-1232.00', description: 'Fix computers when they don’t work.' },
            { title: 'Database Administrator', code: '15-1242.00', description: 'Organize information on computers.' },
            { title: 'Robotics Technician', code: '17-3024.01', description: 'Build and fix robots that help people.' }
        ]
    },
    commerce: {
        riasec: ['E', 'C'],
        ...
        careers: [
            { title: 'Store Manager', code: '11-2022.00', description: 'Run a shop and help customers buy things.' },
            { title: 'Event Planner', code: '13-1121.00', description: 'Plan fun parties and events for people.' },
            { title: 'Bank Teller', code: '43-3071.00', description: 'Help people save and use their money.' },
            { title: 'Real Estate Agent', code: '41-9022.00', description: 'Help people find homes to live in.' },
            { title: 'Business Consultant', code: '13-1111.00', description: 'Give advice to make businesses better.' }
        ]
    },
    engineering: {
        riasec: ['R', 'I'],
        ...
        careers: [
            { title: 'Biomedical Engineer', code: '17-2031.00', description: 'Make tools to help doctors save lives.' },
            { title: 'Environmental Engineer', code: '17-2081.00', description: 'Keep water and air clean for everyone.' },
            { title: 'Robotics Engineer', code: '17-2199.08', description: 'Design robots to do helpful tasks.' },
            { title: 'Structural Engineer', code: '17-2051.01', description: 'Plan strong buildings that don’t fall.' },
            { title: 'Automotive Engineer', code: '17-2141.02', description: 'Design cars that are safe and fast.' }
        ]
    }
};
...
async function loadQuestions(category, retries = 3) {
    ...
    console.log(`[LOAD] Fetching questions: questions-6to8-${category}_tamil.json`);
    for (let attempt = 1; attempt <= retries; attempt++) {
        try {
            const response = await fetch(`questions-6to8-${category}_tamil.json?cachebust=${Date.now()}`, {
                method: 'GET',
                cache: 'no-store'
            });
            ...
            if (attempt === retries) {
                assessmentContent.innerHTML = `<p style="color: #d32f2f;">Error: Unable to load questions for ${categoryDisplayNames[category].tamil}. Check if file 'questions-6to8-${category}_tamil.json' exists.</p>`;
                return [];
            }
...
async function loadActivities(category, retries = 3) {
    ...
    console.log(`[LOAD] Fetching activities: activities-6to8-${category}_tamil.json`);
    for (let attempt = 1; attempt <= retries; attempt++) {
        try {
            const response = await fetch(`activities-6to8-${category}_tamil.json?cachebust=${Date.now()}`, {
                method: 'GET',
                cache: 'no-store'
            });
            if (!response.ok) throw new Error(`HTTP ${response.status}`);
            ...
            console.error(`[LOAD] Attempt ${attempt}/${retries}: ${error.message}`);
            if (attempt === retries) {
                assessmentContent.innerHTML = `<p style="color: #d32f2f;">Error: Unable to load activities for ${categoryDisplayNames[category].tamil}. Check if file 'activities-6to8-${category}_tamil.json' exists.</p>`;
                return [];
            }
            await new Promise(resolve => setTimeout(resolve, 1000));
        }
...
function moveToNextCategoryOrFinish() {
    ...
    try {
        ...
        const htmlFile = isPsychometric ? 'psychometric-assessment-6to8-tamil' : 'activity-assessment-6to8-tamil';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (startActivityBtn) {
        startActivityBtn.addEventListener('click', () => {
            ...
            window.location.href = 'activity-assessment-6to8-tamil.html?category=career_exploration';
        });
    }
