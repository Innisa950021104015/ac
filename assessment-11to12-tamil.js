...
const isPsychometric = document.location.pathname.includes('psychometric') || document.location.pathname.includes('tamil-psychometric-11to12.html');
const isActivity = document.location.pathname.includes('activity') || document.location.pathname.includes('tamil-activity-11to12.html');
...
const careerDomains = {
    arts: {
        ...
        careers: [
            { title: 'Illustrator', code: '27-1024.00', description: 'Create drawings and illustrations for media.' },
            { title: 'Scriptwriter', code: '27-2012.00', description: 'Write scripts for films or TV shows.' },
            { title: 'Dance Instructor', code: '25-1121.00', description: 'Teach dance techniques to students.' },
            { title: 'Creative Writer', code: '29-1129.00', description: 'Write stories or poems for publication.' },
            { title: 'Fashion Designer', code: '27-1025.00', description: 'Design clothing and accessories.' }
        ]
    },
    science: {
        riasec: ['I', 'R'],
        ...
        careers: [
            { title: 'Biomedical Technician', code: '29-1141.00', description: 'Maintain and repair medical equipment.' },
            { title: 'Environmental Scientist', code: '29-1123.00', description: 'Study environmental issues and solutions.' },
            { title: 'Dietitian', code: '29-1051.00', description: 'Advise on nutrition and healthy eating.' },
            { title: 'Forensic Technician', code: '29-2012.00', description: 'Analyze evidence for criminal investigations.' },
            { title: 'Wildlife Biologist', code: '29-1122.00', description: 'Study animals and their habitats.' }
        ]
    },
    computer: {
        riasec: ['I', 'C'],
        ...
        careers: [
            { title: 'Web Developer', code: '15-1252.00', description: 'Build and maintain websites.' },
            { title: 'Game Designer', code: '15-2051.00', description: 'Create concepts for video games.' },
            { title: 'IT Support Specialist', code: '15-1212.00', description: 'Assist users with technical issues.' },
            { title: 'Database Administrator', code: '15-1244.00', description: 'Manage and organize data systems.' },
            { title: 'Mobile App Developer', code: '15-1221.00', description: 'Create applications for smartphones.' }
        ]
    },
    commerce: {
        riasec: ['E', 'C'],
        ...
        careers: [
            { title: 'Advertising Coordinator', code: '11-2021.00', description: 'Plan and execute ad campaigns.' },
            { title: 'Budget Analyst', code: '13-2051.00', description: 'Help organizations manage budgets.' },
            { title: 'Event Planner', code: '11-1011.00', description: 'Organize and manage events.' },
            { title: 'Bookkeeper', code: '13-2011.00', description: 'Record financial transactions.' },
            { title: 'Retail Manager', code: '11-2022.00', description: 'Oversee store operations and sales.' }
        ]
    },
    engineering: {
        riasec: ['R', 'I'],
        ...
        careers: [
            { title: 'Automotive Engineer', code: '17-2141.00', description: 'Design vehicle systems and components.' },
            { title: 'Structural Engineer', code: '17-2051.00', description: 'Design safe buildings and bridges.' },
            { title: 'Robotics Engineer', code: '17-2071.00', description: 'Build and program robotic systems.' },
            { title: 'Environmental Engineer', code: '17-2011.00', description: 'Develop solutions for environmental issues.' },
            { title: 'Biomedical Engineer', code: '17-2041.00', description: 'Design medical devices and equipment.' }
        ]
    }
};
...
async function loadQuestions(category, retries = 3) {
    ...
    console.log(`[LOAD] Fetching questions: questions-11to12-${category}_tamil.json`);
    for (let attempt = 1; attempt <= retries; attempt++) {
        try {
            const response = await fetch(`questions-11to12-${category}_tamil.json?cachebust=${Date.now()}`, {
                method: 'GET',
                cache: 'no-store'
            });
            ...
            if (attempt === retries) {
                assessmentContent.innerHTML = `<p style="color: #d32f2f;">Error: Unable to load questions for ${categoryDisplayNames[category].tamil}. Check if file 'questions-11to12-${category}_tamil.json' exists.</p>`;
                return [];
            }
...
async function loadActivities(category, retries = 3) {
    ...
    console.log(`[LOAD] Fetching activities: activities-11to12-${category}_tamil.json`);
    for (let attempt = 1; attempt <= retries; attempt++) {
        try {
            const response = await fetch(`activities-11to12-${category}_tamil.json?cachebust=${Date.now()}`, {
                method: 'GET',
                cache: 'no-store'
            });
            if (!response.ok) throw new Error(`HTTP ${response.status}`);
            ...
            console.error(`[LOAD] Attempt ${attempt}/${retries}: ${error.message}`);
            if (attempt === retries) {
                assessmentContent.innerHTML = `<p style="color: #d32f2f;">Error: Unable to load activities for ${categoryDisplayNames[category].tamil}. Check if file 'activities-11to12-${category}_tamil.json' exists.</p>`;
                return [];
            }
            await new Promise(resolve => setTimeout(resolve, 1000));
        }
...
function moveToNextCategoryOrFinish() {
    ...
    try {
        ...
        const htmlFile = isPsychometric ? 'psychometric-assessment-11to12-tamil' : 'activity-assessment-11to12-tamil';
...
async function generatePDF() {
    ...
    try {
        ...
        doc.save('Career_Assessment_Report_11to12.pdf');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (startActivityBtn) {
        startActivityBtn.addEventListener('click', () => {
            ...
            window.location.href = 'activity-assessment-11to12-tamil.html?category=career_exploration';
        });
    }
