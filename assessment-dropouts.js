...
console.log('[INIT] English assessment script loaded @ May 29, 2025, 18:00 PM IST');

// Determine if this is a psychometric or activity assessment
const isPsychometric = document.location.pathname.includes('psychometric') || document.location.pathname.includes('english-psychometric-dropouts.html');
const isActivity = document.location.pathname.includes('activity') || document.location.pathname.includes('english-activity-dropouts.html');
...
const careerDomains = {
    arts: {
        ...
        careers: [
            { title: 'Street Artist', code: '27-1013.00', description: 'Paint murals and designs on walls to make places colorful.' },
            { title: 'Jewelry Maker', code: '51-9071.00', description: 'Create necklaces and rings to sell at markets.' },
            { title: 'Tattoo Artist', code: '39-9099.00', description: 'Draw cool designs on people’s skin with ink.' },
            { title: 'Cake Decorator', code: '51-3011.00', description: 'Make cakes look pretty with icing and colors.' },
            { title: 'Florist', code: '27-1023.00', description: 'Arrange flowers to make beautiful bouquets.' }
        ]
    },
    science: {
        riasec: ['I', 'R'],
        ...
        careers: [
            { title: 'Animal Caretaker', code: '39-2021.00', description: 'Feed and care for pets and animals.' },
            { title: 'Lab Assistant', code: '31-9096.00', description: 'Help scientists by cleaning tools and organizing labs.' },
            { title: 'Herbalist', code: '29-9099.00', description: 'Use plants to make natural medicines.' },
            { title: 'Aquarium Worker', code: '39-2021.00', description: 'Feed fish and clean tanks at aquariums.' },
            { title: 'Garden Technician', code: '37-3011.00', description: 'Plant and care for trees and flowers.' }
        ]
    },
    computer: {
        riasec: ['I', 'C'],
        ...
        careers: [
            { title: 'Front-End Coder', code: '15-1256.00', description: 'Build websites using JavaScript to make them look nice.' },
            { title: 'Freelance Web Designer', code: '27-1011.00', description: 'Create website looks for clients from home.' },
            { title: 'Tech Support Helper', code: '15-1232.00', description: 'Help people fix their computer problems.' },
            { title: 'Content Creator', code: '27-3043.00', description: 'Make videos or blogs using tech tools.' },
            { title: 'E-Commerce Seller', code: '41-9099.00', description: 'Sell products online using website skills.' }
        ]
    },
    commerce: {
        riasec: ['E', 'C'],
        ...
        careers: [
            { title: 'Market Vendor', code: '41-9091.00', description: 'Sell fruits or crafts at a market stall.' },
            { title: 'Delivery Driver', code: '53-3033.00', description: 'Deliver packages to people’s homes.' },
            { title: 'Shop Assistant', code: '41-2031.00', description: 'Help customers find items in a store.' },
            { title: 'Food Stall Owner', code: '35-1012.00', description: 'Cook and sell tasty snacks at a stall.' },
            { title: 'Customer Service Agent', code: '43-4051.00', description: 'Answer phone calls to help customers.' }
        ]
    },
    engineering: {
        riasec: ['R', 'I'],
        ...
        careers: [
            { title: 'Carpenter', code: '47-2031.00', description: 'Build furniture and houses with wood.' },
            { title: 'Electrician’s Helper', code: '47-3013.00', description: 'Assist with fixing wires and lights.' },
            { title: 'Plumber’s Apprentice', code: '47-2152.00', description: 'Help fix pipes and water systems.' },
            { title: 'Welder', code: '51-4121.00', description: 'Join metal parts to make strong structures.' },
            { title: 'Bike Mechanic', code: '49-3091.00', description: 'Fix bicycles to keep them running.' }
        ]
    }
};
...
async function loadQuestions(category, retries = 3) {
    ...
    console.log(`[LOAD] Fetching questions: questions-dropouts-${category}_english.json`);
    for (let attempt = 1; attempt <= retries; attempt++) {
        try {
            const response = await fetch(`questions-dropouts-${category}_english.json?cachebust=${Date.now()}`, {
                method: 'GET',
                cache: 'no-store'
            });
            ...
            if (attempt === retries) {
                assessmentContent.innerHTML = `<p style="color: #d32f2f;">Error: Unable to load questions for ${categoryDisplayNames[category]}. Check if file 'questions-dropouts-${category}_english.json' exists.</p>`;
                return [];
            }
...
async function loadActivities(category, retries = 3) {
    ...
    console.log(`[LOAD] Fetching activities: activities-dropouts-${category}_english.json`);
    for (let attempt = 1; attempt <= retries; attempt++) {
        try {
            const response = await fetch(`activities-dropouts-${category}_english.json?cachebust=${Date.now()}`, {
                method: 'GET',
                cache: 'no-store'
            });
            if (!response.ok) throw new Error(`HTTP ${response.status}`);
            ...
            console.error(`[LOAD] Attempt ${attempt}/${retries}: ${error.message}`);
            if (attempt === retries) {
                assessmentContent.innerHTML = `<p style="color: #d32f2f;">Error: Unable to load activities for ${categoryDisplayNames[category]}. Check if file 'activities-dropouts-${category}_english.json' exists.</p>`;
                return [];
            }
            await new Promise(resolve => setTimeout(resolve, 1000));
        }
...
function moveToNextCategoryOrFinish() {
    ...
    try {
        ...
        const htmlFile = isPsychometric ? 'psychometric-assessment-dropouts-english' : 'activity-assessment-dropouts-english';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (startActivityBtn) {
        startActivityBtn.addEventListener('click', () => {
            ...
            window.location.href = 'activity-assessment-dropouts-english.html?category=career_exploration';
        });
    }
    ...
    if (viewPdfLink) {
        viewPdfLink.addEventListener('click', async () => {
            ...
            try {
                ...
                doc.text('For more information about your career options, visit the Career Info page at our website:', 10, y, { maxWidth: 190 });
                y += 5;
                doc.text('https://careerpath.careerinfo.com', 10, y);
                y += 10;
