...
const isPsychometric = document.location.pathname.includes('psychometric') || document.location.pathname.includes('english-psychometric-11to12.html');
const isActivity = document.location.pathname.includes('activity') || document.location.pathname.includes('english-activity-11to12.html');
...
const careerDomains = {
    arts: {
        ...
        careers: [
            { title: 'Graphic Designer', code: '27-1024.00', description: 'Create visual concepts using software.' },
            { title: 'Film Director', code: '27-2012.00', description: 'Direct the production of films.' },
            { title: 'Music Teacher', code: '25-1121.00', description: 'Teach music in schools.' },
            { title: 'Art Therapist', code: '29-1129.00', description: 'Use art to support mental health.' },
            { title: 'Interior Designer', code: '27-1025.00', description: 'Plan and design interior spaces.' }
        ]
    },
    science: {
        riasec: ['I', 'R'],
        ...
        careers: [
            { title: 'Biologist', code: '19-1021.00', description: 'Study living organisms.' },
            { title: 'Chemist', code: '19-2031.00', description: 'Conduct chemical experiments.' },
            { title: 'Environmental Scientist', code: '19-2041.00', description: 'Protect the environment.' },
            { title: 'Physicist', code: '19-2012.00', description: 'Study physical phenomena.' },
            { title: 'Geologist', code: '19-2042.00', description: 'Study earth’s composition.' }
        ]
    },
    computer: {
        riasec: ['I', 'C'],
        ...
        careers: [
            ...
            { title: 'Data Scientist', code: '15-2051.00', description: 'Analyze complex data.' },
            { title: 'Cybersecurity Analyst', code: '15-1212.00', description: 'Protect systems from threats.' },
            { title: 'Network Administrator', code: '15-1244.00', description: 'Manage computer networks.' },
            { title: 'AI Engineer', code: '15-1221.00', description: 'Develop AI systems.' }
        ]
    },
    commerce: {
        riasec: ['E', 'C'],
        ...
        careers: [
            { title: 'Marketing Manager', code: '11-2021.00', description: 'Plan marketing strategies.' },
            { title: 'Financial Analyst', code: '13-2051.00', description: 'Analyze financial data.' },
            { title: 'Entrepreneur', code: '11-1011.00', description: 'Start and manage businesses.' },
            { title: 'Accountant', code: '13-2011.00', description: 'Manage financial records.' },
            { title: 'Sales Manager', code: '11-2022.00', description: 'Lead sales teams.' }
        ]
    },
    engineering: {
        riasec: ['R', 'I'],
        ...
        careers: [
            ...
            { title: 'Civil Engineer', code: '17-2051.00', description: 'Design infrastructure projects.' },
            { title: 'Electrical Engineer', code: '17-2071.00', description: 'Work with electrical systems.' },
            { title: 'Aerospace Engineer', code: '17-2011.00', description: 'Design aircraft and spacecraft.' },
            { title: 'Chemical Engineer', code: '17-2041.00', description: 'Develop chemical processes.' }
        ]
    }
};
...
async function loadQuestions(category, retries = 3) {
    ...
    console.log(`[LOAD] Fetching questions: questions-11to12-${category}_english.json`);
    for (let attempt = 1; attempt <= retries; attempt++) {
        try {
            const response = await fetch(`questions-11to12-${category}_english.json?cachebust=${Date.now()}`, {
                method: 'GET',
                cache: 'no-store'
            });
            ...
            if (attempt === retries) {
                assessmentContent.innerHTML = `<p style="color: #d32f2f;">Error: Unable to load questions for ${categoryDisplayNames[category]}. Check if file 'questions-11to12-${category}_english.json' exists.</p>`;
                return [];
            }
...
async function loadActivities(category, retries = 3) {
    ...
    console.log(`[LOAD] Fetching activities: activities-11to12-${category}_english.json`);
    for (let attempt = 1; attempt <= retries; attempt++) {
        try {
            const response = await fetch(`activities-11to12-${category}_english.json?cachebust=${Date.now()}`, {
                method: 'GET',
                cache: 'no-store'
            });
            if (!response.ok) throw new Error(`HTTP ${response.status}`);
            ...
            console.error(`[LOAD] Attempt ${attempt}/${retries}: ${error.message}`);
            if (attempt === retries) {
                assessmentContent.innerHTML = `<p style="color: #d32f2f;">Error: Unable to load activities for ${categoryDisplayNames[category]}. Check if file 'activities-11to12-${category}_english.json' exists.</p>`;
                return [];
            }
            await new Promise(resolve => setTimeout(resolve, 1000));
        }
...
function moveToNextCategoryOrFinish() {
    ...
    try {
        ...
        const htmlFile = isPsychometric ? 'psychometric-assessment-11to12-english' : 'activity-assessment-11to12-english';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (startActivityBtn) {
        startActivityBtn.addEventListener('click', () => {
            ...
            window.location.href = 'activity-assessment-11to12-english.html?category=career_exploration';
        });
    }
