...
const isPsychometric = document.location.pathname.includes('psychometric') || document.location.pathname.includes('tamil-psychometric-9to10.html');
const isActivity = document.location.pathname.includes('activity') || document.location.pathname.includes('tamil-activity-9to10.html');
...
const careerDomains = {
    arts: {
        ...
        careers: [
            { title: 'Musician', code: '27-2042.00', description: 'Play instruments to make beautiful music.' },
            { title: 'Animator', code: '27-1014.00', description: 'Create moving cartoons for movies and games.' },
            { title: 'Fashion Designer', code: '27-1022.00', description: 'Design clothes that people love to wear.' },
            { title: 'Puppeteer', code: '27-2012.03', description: 'Use puppets to tell fun stories.' },
            { title: 'Illustrator', code: '27-1013.00', description: 'Draw pictures for books and magazines.' }
        ]
    },
    science: {
        riasec: ['I', 'R'],
        ...
        careers: [
            { title: 'Astronomer', code: '19-2011.00', description: 'Study stars and planets in space.' },
            { title: 'Geologist', code: '19-2042.00', description: 'Explore rocks and learn about the Earth.' },
            { title: 'Marine Biologist', code: '19-1023.00', description: 'Study fish and animals in the ocean.' },
            { title: 'Zoologist', code: '19-1023.00', description: 'Learn about animals and how they live.' },
            { title: 'Meteorologist', code: '19-2021.00', description: 'Predict the weather to keep people safe.' }
        ]
    },
    computer: {
        riasec: ['I', 'C'],
        ...
        careers: [
            { title: 'App Developer', code: '15-1253.00', description: 'Make apps for phones and tablets.' },
            { title: 'UX Designer', code: '15-1255.01', description: 'Design websites that are easy to use.' },
            { title: 'Cloud Engineer', code: '15-1299.02', description: 'Store information safely on the internet.' },
            { title: 'Computer Repair Technician', code: '49-2011.00', description: 'Fix computers and make them work again.' },
            { title: 'Virtual Reality Developer', code: '15-1256.00', description: 'Create fun virtual worlds for games.' }
        ]
    },
    commerce: {
        riasec: ['E', 'C'],
        ...
        careers: [
            { title: 'Restaurant Manager', code: '11-9051.00', description: 'Run a restaurant and serve tasty food.' },
            { title: 'Travel Agent', code: '41-3041.00', description: 'Plan exciting trips for people.' },
            { title: 'Insurance Agent', code: '41-3021.00', description: 'Help people stay safe with insurance.' },
            { title: 'Retail Buyer', code: '13-1022.00', description: 'Choose products for stores to sell.' },
            { title: 'Advertising Coordinator', code: '11-2011.00', description: 'Create ads to tell people about products.' }
        ]
    },
    engineering: {
        riasec: ['R', 'I'],
        ...
        careers: [
            { title: 'Agricultural Engineer', code: '17-2021.00', description: 'Design tools to help farmers grow food.' },
            { title: 'Petroleum Engineer', code: '17-2171.00', description: 'Find oil to make energy for cars.' },
            { title: 'Nuclear Engineer', code: '17-2161.00', description: 'Use energy to make power for homes.' },
            { title: 'Materials Engineer', code: '17-2131.00', description: 'Create strong materials for buildings.' },
            { title: 'Acoustical Engineer', code: '17-2199.01', description: 'Design spaces with clear sound.' }
        ]
    }
};
...
async function loadQuestions(category, retries = 3) {
    ...
    console.log(`[LOAD] Fetching questions: questions-9to10-${category}_tamil.json`);
    for (let attempt = 1; attempt <= retries; attempt++) {
        try {
            const response = await fetch(`questions-9to10-${category}_tamil.json?cachebust=${Date.now()}`, {
                method: 'GET',
                cache: 'no-store'
            });
...
async function loadActivities(category, retries = 3) {
    ...
    console.log(`[LOAD] Fetching activities: activities-9to10-${category}_tamil.json`);
    for (let attempt = 1; attempt <= retries; attempt++) {
        try {
            const response = await fetch(`activities-9to10-${category}_tamil.json?cachebust=${Date.now()}`, {
                method: 'GET',
                cache: 'no-store'
            });
            if (!response.ok) throw new Error(`HTTP ${response.status}`);
...
function moveToNextCategoryOrFinish() {
    ...
    try {
        ...
        const htmlFile = isPsychometric ? 'psychometric-assessment-9to10-tamil' : 'activity-assessment-9to10-tamil';
...
async function generatePDF() {
    ...
    try {
        ...
        doc.save('Career_Assessment_Report_9to10.pdf');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (startActivityBtn) {
        startActivityBtn.addEventListener('click', () => {
            ...
            window.location.href = 'activity-assessment-9to10-tamil.html?category=career_exploration';
        });
    }
