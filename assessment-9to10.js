...
const isPsychometric = document.location.pathname.includes('psychometric') || document.location.pathname.includes('psychometric-assessment-9to10.html');
const isActivity = document.location.pathname.includes('activity') || document.location.pathname.includes('activity-assessment-9to10.html');
...
async function loadQuestions(category, retries = 3) {
    ...
    console.log(`[LOAD] Fetching questions: questions-9to10-${category}_english.json`);
    for (let attempt = 1; attempt <= retries; attempt++) {
        try {
            const response = await fetch(`questions-9to10-${category}_english.json?cachebust=${Date.now()}`, {
                method: 'GET',
                cache: 'no-store'
            });
            ...
            if (attempt === retries) {
                assessmentContent.innerHTML = `<p style="color: #d32f2f;">Error: Unable to load questions for ${categoryDisplayNames[category]}. Check if file 'questions-9to10-${category}_english.json' exists.</p>`;
                return [];
            }
...
async function loadActivities(category, retries = 3) {
    ...
    console.log(`[LOAD] Fetching activities: activities-9to10-${category}_english.json`);
    for (let attempt = 1; attempt <= retries; attempt++) {
        try {
            const response = await fetch(`activities-9to10-${category}_english.json?cachebust=${Date.now()}`, {
                method: 'GET',
                cache: 'no-store'
            });
            if (!response.ok) throw new Error(`HTTP ${response.status}`);
            ...
            console.error(`[LOAD] Attempt ${attempt}/${retries}: ${error.message}`);
            if (attempt === retries) {
                assessmentContent.innerHTML = `<p style="color: #d32f2f;">Error: Unable to load activities for ${categoryDisplayNames[category]}. Check if file 'activities-9to10-${category}_english.json' exists.</p>`;
                return [];
            }
            await new Promise(resolve => setTimeout(resolve, 1000));
        }
...
function moveToNextCategoryOrFinish() {
    ...
    try {
        ...
        const htmlFile = isPsychometric ? 'psychometric-assessment-9to10' : 'activity-assessment-9to10';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (startActivityBtn) {
        startActivityBtn.addEventListener('click', () => {
            ...
            window.location.href = 'activity-assessment-9to10.html?category=career_exploration';
        });
    }
